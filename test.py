...
# %%
class GLOWRaycast:
    def __init__(self, time: datetime, lat: float, lon: float, heading: float, max_alt: float = 1000, n_pts: int = 200, n_bins: int = 100, *, n_threads: int = 24, full_circ: bool = False):
        ...
        max_d = 6400 * np.pi if full_circ else EARTH_RADIUS * \
            np.arccos(EARTH_RADIUS / (EARTH_RADIUS + max_alt)) # find maximum distance where LOS intersects exobase # 6400 * np.pi
        distpts = np.linspace(0, max_d, n_pts, endpoint=True) # points on the earth where we need to sample
        self._locs = []
    ...
    @classmethod
    def no_precipitation(cls, time: datetime, lat: float, lon: float, heading: float, max_alt: float = 1000, n_pts: int = 200, n_bins: int = 100, *, n_threads: int = 24, full_output: bool = False) -> typing.Any:
        ...
        """
        grobj = cls(time, lat, lon, heading, max_alt, n_pts, n_bins, n_threads=n_threads)
        bds = grobj.run()
        iono = grobj.transform_coord()
        if not full_output:
            return iono
        else: return (iono, bds)
    ...
    def run(self)->xr.Dataset:
        ...
        if self._bds is not None:
            return self._bds
        self._bds = self._calc_glow() # calculate the GLOW model for each lat-lon point determined in init()
    ...
    def transform_coord(self)->xr.Dataset:
        ...
        self._tmin, self._tmax = 0, tt.max() # highest and lowest look angle (90 deg - za)
        self._nr_num = len(self._bds.alt_km.values)*2 # resample to double density
        self._nt_num = len(self._bds.angle.values)*2 # resample to double density
        ...
        self._nr = nr = np.linspace(rmin, rmax, self._nr_num, endpoint=True) # local r
        self._nt = nt = np.linspace(tmin, tmax, self._nt_num, endpoint=True) # local look angle
        self._ntt, self._nrr = self._get_global_coords(nt, nr) # get meshgrid of the R, T coord system from regular r, la grid
        ...
        self._ntt = (self._ntt - self._theta.min()) / \
            (self._theta.max() - self._theta.min()) * len(self._theta) # calculate equivalent index (pixel coord) from original T grid
        self._nrr = (self._nrr - self._altkm.min() - EARTH_RADIUS) / \
            (self._altkm.max() - self._altkm.min()) * len(self._altkm) # calculate equivalent index (pixel coord) from original R (alt_km) grid
        data_vars = {}
        bds = self._bds
        ...
        for key in single_keys: # map all the single key types from (angle, alt_km) -> (la, r)
        ...
        iono = xr.Dataset(data_vars=data_vars, coords={'za': np.pi/2 - nt, 'r': nr}) # dataset of (angle, alt_km) vars
        ver = []
        for key in coord_wavelength: # map all the wavelength data from (angle, alt_km, wavelength) -> (la, r, wavelength)
        ...
        ver = xr.DataArray(
            ver,
            coords={'za': np.pi/2 - nt, 'r': nr, 'wavelength': coord_wavelength},
        ...
        ) # calculate (angle, alt_km, wavelength) -> (la, r, wavelength) dataset
        ...
        _rr, _ = self._get_local_coords(bds.angle.values, np.ones(bds.angle.values.shape)*EARTH_RADIUS)
        _rr = rr[:, 0] # spatial EGrid
        d = []
        for en in coord_energy: # for each energy
            inp = bds['precip'].loc[dict(energy=en)].values
            out = np.interp(nr, _rr, inp) # interpolate to appropriate energy grid
            d.append(out)
        d = np.asarray(d).T
        precip = xr.Dataset({'precip': (('r', 'energy'), d)}, coords={'r': nr, 'energy': coord_energy})
    ...
    def _global_from_local(self, pt: tuple(int, int))->tuple(float, float): # get global coord index from local coord index, implemented as LUT
    ...
    def _get_global_coords(self, t: np.ndarray | float, r: np.ndarray | float, r0: float = EARTH_RADIUS, meshgrid: bool = True) -> tuple(np.ndarray, np.ndarray):
        if isinstance(r, np.ndarray) and (t, np.ndarray): # if array
            ...
            raise RuntimeError
        _t = np.pi/2 - _t
        rr = np.sqrt((_r*np.cos(_t) + r0)**2 + (_r*np.sin(_t))**2) # r, la to R, T
    ...
    def _get_local_coords(self, t: np.ndarray | float, r: np.ndarray | float, r0: float = EARTH_RADIUS) -> tuple(np.ndarray, np.ndarray):
        if isinstance(r, np.ndarray) and (t, np.ndarray):
            ...
            raise RuntimeError
        rr = np.sqrt((_r*np.cos(_t) - r0)**2 + (_r*np.sin(_t))**2) # R, T to r, la
    ...
    def _calc_glow_single(self, index): # calculate glow model for one location
        d = self._locs[index]
        return glow.no_precipitation(self._time, d[0], d[1], self._nbins)

    def _calc_glow(self) -> xr.Dataset: # run glow model calculation
        self._dss = process_map(self._calc_glow_single, range(
...
fig, ax = plt.subplots(dpi=300, subplot_kw=dict(projection='polar'))
tn = bds.Tn.values
r, t = bds.alt_km.values, bds.angle.values  # np.meshgrid((alt + ofst) / ofst, ang)
print(r.shape, t.shape, tn.shape)
# , extent=[0, 0, 7400 / EARTH_RADIUS, ang.max()])
...
fig, ax = plt.subplots(dpi=300, subplot_kw=dict(projection='polar'))
tn = iono.Tn.values
r, t = iono.r.values, np.pi/2 - iono.za.values  # np.meshgrid((alt + ofst) / ofst, ang)
print(r.shape, t.shape)
r, t = np.meshgrid(r, t)
# , extent=[0, 0, 7400 / EARTH_RADIUS, ang.max()])
im = ax.contourf(t, r, tn, 100)
